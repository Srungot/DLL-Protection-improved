...
bool FillPEHeaderz(ULONG_PTR BaseAddress, OUT PE_HEADER& PEHeader)
{
    if (!IsValidPEHeaderz(BaseAddress))
        return false;
...
bool FillRemotePEHeaderz(HANDLE ProcessHandle, ULONG_PTR BaseAddress, OUT REMOTE_PE_HEADER& PEHeader)
{
    ...
    if (!ReadProcessMemory(ProcessHandle, PVOID(BaseAddress), PEHeader.rawData, PE_HEADER_SIZE, NULL))
        return false;
    if (!FillPEHeaderz(ULONG_PTR(&PEHeader.rawData), PEHeader))
        return false;
...
bool IsValidPEHeaderz(ULONG_PTR BaseAddress)
{
...
const PIMAGE_SECTION_HEADER GetPeSectionByNamez(const PE_HEADER& HeaderData, const char* SectionName)
{
...
DWORD GetSizeOfImagez(PVOID BaseAddress)
{
    if (!IsValidPEHeaderz(ULONG_PTR(BaseAddress)))
        return 0;
