...
{
    PE_HEADER pe;
    if (!FillPEHeaderz(SIZE_T(ImageBase), pe))
        return false;

    const PIMAGE_SECTION_HEADER text = GetPeSectionByNamez(pe, _xor_(".text").c_str());
    const PIMAGE_SECTION_HEADER rdata = GetPeSectionByNamez(pe, _xor_(".rdata").c_str());
    const PIMAGE_SECTION_HEADER data = GetPeSectionByNamez(pe, _xor_(".data").c_str());
...
int selfcode()
{
    ...
    {
        const DWORD imageSize = GetSizeOfImagez(imagebase);
